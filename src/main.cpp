...
int main(int argc, char **argv) {
  ...
  float cuboid_dia_max =
      sqrt(4 * marker_size * marker_size + tolerance * tolerance);

  float cuboid_area_min = sqrt(marker_size * marker_size);
  float cuboid_area_max =
      sqrt(2 * marker_size * marker_size + tolerance * tolerance);
  ...
  // visualization::PCLVisualizer::Ptr viewer(new
  // visualization::PCLVisualizer("3d view"));
  boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(
      new visualization::PCLVisualizer("3d view"));
  ...
  // viewer->addText("Step3: filter out impractical
  // candidates",10,10,"text4",v4); viewer->addText("Step4: conduct marker
  // detection in ROI",10,10,"text5",v5); viewer->addText("Step5: output the
  // vertices of the detected marker",10,10,"text6",v6);

  // viewer->addCoordinateSystem();

  pcl::visualization::PointCloudColorHandlerGenericField<pcl::PointXYZI>
      fildColor_raw(cloud, "intensity");
  viewer->addPointCloud(cloud, fildColor_raw, "raw point cloud", v1);
  viewer->setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 0.25, "raw point cloud");
  ...
  if (cloud->size() & 1) {
    ...
    for (int i = 0; i < len; ++i) {
      ...
      float magnitude = std::sqrt(g_est[0] * g_est[0] + g_est[1] * g_est[1] +
                                  g_est[2] * g_est[2]);
      if (!isnan(magnitude))
        q.push(magnitude);
    }
  ...
  for (size_t p = 0; p < cloud->size(); ++p) {
    ...
    if (!isnan(magnitude))
      ...
      if (magnitude > intensity_threshold) {
        ++counter;
        valid_point = {cloud->points[p].x, cloud->points[p].y,
                       cloud->points[p].z, cloud->points[p].intensity};
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr intensity_feature(
      new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI> &intensity_feature_pointcloud =
      *intensity_feature;
  ...
  visualization::PointCloudColorHandlerCustom<pcl::PointXYZI> rgb2(
      intensity_feature, 0, 255, 0); // feature cloud
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it =
    ...
    float max1, max2;
    float OBB_dia = sqrt(OBB_width * OBB_width + OBB_height * OBB_height +
                         OBB_depth * OBB_depth);
    ...
    // test
    viewer->addCube(position, quat, OBB_width, OBB_height, OBB_depth,
                    "OBBoriginal" + 2 * jj, v3);
    ...
    if (max1 / max2 < ratio_threshold && max2 / max1 > 1 / ratio_threshold &&
      ...
      pcl::PointCloud<pcl::PointXYZI>::Ptr points_in_box{
          new pcl::PointCloud<pcl::PointXYZI>};
      pcl::PointCloud<pcl::PointXYZI>::Ptr points_in_box_transformed{
          new pcl::PointCloud<pcl::PointXYZI>};
      pcl::PointCloud<pcl::PointXYZI>::Ptr points_in_box_transformed_2{
          new pcl::PointCloud<pcl::PointXYZI>};

      pcl::CropBox<pcl::PointXYZI> box_filter;
      Eigen::Vector3f minpoint(min_point_OBB.x, min_point_OBB.y,
                               min_point_OBB.z);
      Eigen::Vector3f maxpoint(max_point_OBB.x, max_point_OBB.y,
                               max_point_OBB.z);
      ...
      box_filter.setMin(Eigen::Vector4f(buffer * min_point_OBB.x,
                                        buffer * min_point_OBB.y,
                                        buffer * min_point_OBB.z, 1.0));
      box_filter.setMax(Eigen::Vector4f(buffer * max_point_OBB.x,
                                        buffer * max_point_OBB.y,
                                        buffer * max_point_OBB.x, 1.0));
      ...
      pcl::transformPointCloud(*points_in_box, *points_in_box_transformed,
                               transform);

      //	viewer->addPointCloud(points_in_box_transformed,
      //fildColor_transformed, "OBB3"+j);

      pcl::transformPointCloud(*points_in_box_transformed,
                               *points_in_box_transformed, transform_2);
      ...
      pcl::PointCloud<pcl::PointXYZI>::Ptr unique(
          new pcl::PointCloud<pcl::PointXYZI>);
      pcl::PointCloud<pcl::PointXYZI> &unique_cloud = *unique;

      pcl::PointCloud<pcl::PointXYZI>::Ptr unique_i(
          new pcl::PointCloud<pcl::PointXYZI>);
      ...
      unique_cloud_i.points.resize(unique_cloud_i.width *
                                   unique_cloud_i.height);
      ...
      for (size_t p = 0; p < unique_points_i.size(); ++p) {
        ...
        if (unique_points_i[p][3] != 0) {
          unique_cloud_i.points[p].x =
              unique_points_i[p][0] * unique_points_i[p][3];
          unique_cloud_i.points[p].y =
              unique_points_i[p][1] * unique_points_i[p][3];
          unique_cloud_i.points[p].z =
              unique_points_i[p][2] * unique_points_i[p][3];
      ...
      float angularResolution =
          (float)(ang_resolution * (M_PI / 180.0f)); //   1.0 degree in radians
      float maxAngleWidth =
          (float)(120.0f * (M_PI / 180.0f)); // 360.0 degree in radians
      float maxAngleHeight =
          (float)(120.0f * (M_PI / 180.0f)); // 180.0 degree in radians
      ...
      range_image_i.createFromPointCloud(
          unique_cloud_i, angularResolution, maxAngleWidth, maxAngleHeight,
          sensorPose, coordinate_frame, noiseLevel, minRange, borderSize);

      range_image.createFromPointCloud(
          unique_cloud, angularResolution, maxAngleWidth, maxAngleHeight,
          sensorPose, coordinate_frame, noiseLevel, minRange, borderSize);

      float *ranges =
          range_image_i.getRangesArray(); // it is the intensity array actually
      float max = 0;
      float val;
...
#pragma omp parallel for
      ...
      if (markerIds.size() > 0) {
        if (markerIds.at(0) > 500) {
          ...
          cv::aruco::detectMarkers(gray_flip, dictionary, markerCorners,
              ...
              if (range_image.isObserved(
...

                {
                  ...
                  range_image.calculate3DPoint(
                      round(markerCorners.at(l)[ll].x),
                      round(markerCorners.at(l)[ll].y + mm), point_up);
                  range_image.calculate3DPoint(
                      round(markerCorners.at(l)[ll].x),
                      round(markerCorners.at(l)[ll].y - mm), point_down);
                  ratio = point_down.range / point_up.range;
                  Eigen::Vector3f temp_vertex(
                      point_up.x * (1 / (1 + ratio)) +
                          point_down.x * (ratio / (1 + ratio)),
                      point_up.y * (1 / (1 + ratio)) +
                          point_down.y * (ratio / (1 + ratio)),
                      point_up.z * (1 / (1 + ratio)) +
                          point_down.z * (ratio / (1 + ratio)));

                  Eigen::Vector3f final_vertex;
                  final_vertex = R_inv * temp_vertex + translationback;

                  final_vertex =
                      rotational_matrix_OBB * final_vertex + position;

                  std::vector<float> final_vertex_std;
                  final_vertex_std = {final_vertex[0], final_vertex[1],
                                      final_vertex[2]};
      ...
      pcl::visualization::PointCloudColorHandlerGenericField<pcl::PointXYZI>
          fildColor(points_in_box, "intensity");
      pcl::visualization::PointCloudColorHandlerGenericField<pcl::PointXYZI>
          fildColor_transformed(points_in_box_transformed, "intensity");
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr vertex_cloud_ptr(
  ...
  visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> rgb3(
      vertex_cloud_ptr, 255, 0, 0); // feature cloud

  viewer->addPointCloud(vertex_cloud_ptr, rgb3, "vertex_cloud", v6);
  viewer->setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 8, "vertex_cloud");
  ...
  if (id_num.size() != 0) {
    ...
    for (int i = 0; i < id_num.size(); i++) {
      ...
      std::cout << "Location of the vertices with respect to the world "
                   "coordinate system (m):"
                << endl;
      std::cout << "first vertex" << "\t" << "x=" << pts1[4 * i][0] << "\t"
                << "y=" << pts1[4 * i][1] << "\t" << "z=" << pts1[4 * i][2]
                << "\n";
      std::cout << "second vertex" << "\t" << "x=" << pts1[4 * i + 1][0] << "\t"
                << "y=" << pts1[4 * i + 1][1] << "\t"
                << "z=" << pts1[4 * i + 1][2] << "\n";
      std::cout << "thrid vertex" << "\t" << "x=" << pts1[4 * i + 2][0] << "\t"
                << "y=" << pts1[4 * i + 2][1] << "\t"
                << "z=" << pts1[4 * i + 2][2] << "\n";
      std::cout << "fourth vertex" << "\t" << "x=" << pts1[4 * i + 3][0] << "\t"
                << "y=" << pts1[4 * i + 3][1] << "\t"
                << "z=" << pts1[4 * i + 3][2] << "\n";
    }
  }
